...
exports.uploadMarketMediaFromBuffer = uploadMarketMediaFromBuffer;
...
async function uploadImageFromBuffer(buffer, filename) {
    return new Promise((resolve, reject) => {
        const stream = cloudinary_1.cloudinary.uploader.upload_stream({ folder: "goatfun/images", resource_type: "image", public_id: path_1.default.parse(filename).name }, (error, result) => {
            if (error || !result)
                return reject(new Error(String(error)));
            resolve({ public_id: result.public_id, secure_url: result.secure_url });
        });
...
async function uploadMarketMediaFromBuffer(buffer, filename) {
    return new Promise((resolve, reject) => {
        const stream = cloudinary_1.cloudinary.uploader.upload_stream({ folder: "goatfun/markets", resource_type: "auto", public_id: path_1.default.parse(filename).name }, (error, result) => {
            if (error || !result)
                return reject(new Error(String(error)));
            resolve({ public_id: result.public_id, secure_url: result.secure_url });
        });
